"""Training config and Helper Classes"""
...
class Config(NamedTuple):
    ...
    # linearly increasing learning rate from zero to the specified final value(5e-5)
    warmup: float = 0.1
...
class Trainer(object):
    ...
    def __init__(self, cfg, model, data_iter, optimizer, save_dir, device):
        self.cfg = cfg # config for training
        self.model = model
        self.data_iter = data_iter # iterator to load data
        self.optimizer = optimizer # used the ScheduledOptim class
    ...
    def train(self, get_loss, model_file=None, pretrain_file=None, data_parallel=True):
        ...
        for e in range(self.cfg.n_epochs): # total loop
            loss_sum = 0. # the sum of iteration losses to get average loss in every epoch
            iter_bar = tqdm(self.data_iter, dec="Iter (loss=X.XX)")
            for i, batch in enumerate(iter_bar): # epoch loop
                ...
                global_step += 1
                loss_sum += loss.item()
                iter_bar.set_description('Iter (loss=%5.3f)' %loss.item()) # update loss display

                if global_step % self.cfg.save_steps == 0: # save at save_steps
    ...
    def eval(self, evaluate, model_file, data_parallel=True):
        ...
        for batch in iter_bar:
            ...
            results.append(result) # append results for each batch
    ...
    def load(self, model_file, pretrain_file):
        ...
        if model_file:
            ...
            self.model.load_state_dict(torch.load(model_file)) # load model object and its params/buffers
            ...
            if pretrain_file.endswith('.ckpt'): # checkpoint file in tensorflow
                ...
                ) # load only transformer parts, starts after 12th position
